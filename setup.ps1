...
# Обычно этот скрипт не нужен: приложение само предложит скачать модель при
# первом запуске. Скрипт полезен для офлайн-подготовки папки и для сборки из
# исходников (чтобы cargo test transcribe_sample нашёл модель).
#
