...
While dictating it shows a waveform overlay (Wispr Flow–style) **without
...
End users don't need to install anything (the exe is self-contained). You only
need the model:

```powershell
powershell -ExecutionPolicy Bypass -File setup.ps1   # downloads the model (~23 MB)
run.bat                                               # launch
```

A microphone tray icon appears. Put the cursor in any text field, say
...
| `ui.rs` | WinAPI: tray, menu, waveform overlay, settings window, hotkey, DPI |
...
| Color (microphone) | Meaning |
|---|---|
| 🔵 blue | Waiting for the wake word |
| 🟢 green | Dictating |
| ⚪ gray | Listening disabled |
